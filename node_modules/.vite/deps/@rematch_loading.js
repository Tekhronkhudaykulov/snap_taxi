...
// node_modules/@rematch/loading/dist/loading.esm.js
function _extends() {
...
var createLoadingAction = function createLoadingAction2(converter, i, cntState) {
  return function(state, payload) {
    var _extends2, _extends3, _extends4;
    var _ref = payload || {
      name: "",
      action: ""
    }, name = _ref.name, action = _ref.action, detailedPayload = _ref.detailedPayload;
    cntState.global += i;
    cntState.models[name] += i;
    cntState.effects[name][action] += i;
    return _extends({}, state, {
      global: converter(cntState.global, detailedPayload),
      models: _extends({}, state.models, (_extends2 = {}, _extends2[name] = converter(cntState.models[name], detailedPayload), _extends2)),
      effects: _extends({}, state.effects, (_extends4 = {}, _extends4[name] = _extends({}, state.effects[name], (_extends3 = {}, _extends3[action] = converter(cntState.effects[name][action], detailedPayload), _extends3)), _extends4))
    });
  };
};
var validateConfig = function validateConfig2(config) {
  if (true) {
    ...
    if (config.asNumber) {
      console.warn(["@rematch/loading deprecation warning:", "\n", '"asNumber" property from @rematch/loading is deprecated, consider replacing "asNumber" to "type": "number".', "\n", 'In future Rematch versions, "asNumber" will be removed.'].join(" "));
    }
...
function assignExtraPayload(insert, error, success) {
  return insert ? {
    error,
    success
  } : null;
...
var index = function(config) {
  ...
  var isAsNumber = config.type === "number";
  var isAsDetailed = config.type === "full";
  var converter = function converter2(cnt, detailedPayload) {
    if (isAsNumber)
      return cnt;
    if (isAsDetailed && detailedPayload) {
      return _extends({}, detailedPayload, {
        loading: cnt > 0
      });
    }
    if (isAsDetailed) {
  ...
  return {
    ...
    onModel: function onModel(_ref2, rematch) {
      ...
      Object.keys(modelActions).forEach(function(action) {
        ...
        var effectWrapper = function effectWrapper2() {
          try {
            ...
            if (effectResult != null && effectResult.then) {
              return effectResult.then(function(r) {
                ...
                rematch.dispatch[loadingModelName].hide({
                  name,
                  action,
                  detailedPayload: assignExtraPayload(isAsDetailed, err, false)
                });
                throw err;
              });
            }
...
//# sourceMappingURL=@rematch_loading.js.map
